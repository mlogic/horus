...
int
ea_cache_add (uint64_t inode_num, struct horus_ea_config *config_ptr)
{
...
int
ea_cache_find (uint64_t inode_num, EA_CACHE_ENTRY ** temp_entry)
{
  int ret = 0;
  ...
  if (fd < 0)
  ...
  else
    {
      ...
      horus_set_kht (fd,
                     ...
                     MIN_CHUNK_SIZE, kht_branching_factor);
          ...
          if ((start >= start_block) && (end <= end_block))
            {
              ...
              if (ret == 0)
                {
                  key_out.keys.keys_val[i].key =
                    strdup (print_key (key, key_len));
                  if (debug)
