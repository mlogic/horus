...
int horus_ea_config_init(struct horus_ea_config *config)
{
...
int horus_ea_config_masterkey(char *path, int in_fd, char *key)
{
	...
	res = fgetxattr(fd, HORUS_EA_NAME, (unsigned char *)&config, HORUS_EA_SIZE, 0);
	if (res == -1)
	{
...
set_attr:
		res = fsetxattr(fd, HORUS_EA_NAME, (unsigned char *)&config, HORUS_EA_SIZE, 0);
	if (res == -1)
		return -errno;
	if (in_fd == -1)
	{
...
int horus_ea_config_add_entry(char *path, int in_fd, struct in_addr ip, uint32_t start_block, uint32_t end_block)
{
	uint32_t count;
	struct horus_ea_config config;
	int fd,res =0;
...
int horus_ea_config_show(char *path)
{
	...
	res = lgetxattr(path, HORUS_EA_NAME, (unsigned char *)&config, HORUS_EA_SIZE, 0);
